...
写了一个背单词软件，用链表做了一个收藏单词表，并且比较得意的是写了一个选择题功能，虽然非常基础，并且没什么AI的感觉，不过还是蛮高兴，拿了98分。代码如下：
...
词库的格式没什么好说，第一行中文，第二行英文，记得去掉txt文件末尾的空格。之后又新写了个功能，把大小写字母全换为小写，并且去除字符串中的符号，代码如下：

```c++
#include <stdio.h>
#define n 100
static char * trans(char * input,int mode)
{
    int j=0;
